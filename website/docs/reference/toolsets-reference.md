...
| `kaspa` | `kaspa_api_health`, `kaspa_network_info`, `kaspa_blockdag_info`, `kaspa_coin_supply`, `kaspa_circulating_coin_supply`, `kaspa_total_coin_supply`, `kaspa_fee_estimate`, `kaspa_price`, `kaspa_marketcap`, `kaspa_hashrate`, `kaspa_max_hashrate`, `kaspa_hashrate_history`, `kaspa_kaspad_info`, `kaspa_blockreward`, `kaspa_halving_info`, `kaspa_virtual_chain_blue_score`, `kaspa_node_rpc_tcp_health`, `kaspa_node_info`, `kasia_indexer_health`, `kaspa_address_balance`, `kaspa_address_name`, `kaspa_address_transaction_count`, `kaspa_address_transactions`, `kaspa_address_utxo_count`, `kaspa_address_utxos`, `kaspa_block_lookup`, `kaspa_blocks`, `kaspa_blocks_from_bluescore`, `kaspa_transaction_lookup`, `kaspa_transaction_count`, `kaspa_virtual_chain`, `kns_domain_owner`, `kns_primary_name`, `kns_search_assets`, `kasia_indexer_handshakes_by_sender`, `kasia_indexer_handshakes_by_receiver`, `kasia_indexer_payments_by_sender`, `kasia_indexer_payments_by_receiver`, `kasia_indexer_contextual_messages_by_sender`, `kasia_indexer_self_stash_by_owner` | Read-only Kaspa, KNS, and Kasia indexer lookups. Defaults to public read APIs but can be pointed at private mirrors with `KASPA_API_URL`, `KNS_API_URL`, and `KASIA_INDEXER_URL`; node RPC coverage includes TCP reachability and a read-only local node-info probe/facade via `KASPA_NODE_RPC_HOST`/`KASPA_NODE_RPC_PORT`, wRPC port `17110`, or `KASPA_NODE_RPC_URL`. |
