...
| `kaspa` | `kaspa_api_health`, `kaspa_node_rpc_tcp_health`, `kasia_indexer_health`, `kaspa_address_balance`, `kaspa_address_name`, `kaspa_address_utxo_count`, `kns_domain_owner`, `kns_primary_name`, `kns_search_assets`, `kasia_indexer_handshakes_by_sender`, `kasia_indexer_handshakes_by_receiver`, `kasia_indexer_payments_by_sender`, `kasia_indexer_payments_by_receiver`, `kasia_indexer_contextual_messages_by_sender`, `kasia_indexer_self_stash_by_owner` | Read-only Kaspa, KNS, and Kasia indexer lookups. Defaults to public read APIs but can be pointed at private mirrors with `KASPA_API_URL`, `KNS_API_URL`, and `KASIA_INDEXER_URL`; node RPC coverage is currently TCP reachability only via `KASPA_NODE_RPC_HOST`/`KASPA_NODE_RPC_PORT`. |
