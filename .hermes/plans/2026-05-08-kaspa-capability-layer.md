...
- Added `kaspa_price`, `kaspa_marketcap`, `kaspa_hashrate`, `kaspa_blockreward`, `kaspa_halving_info`, and `kaspa_virtual_chain_blue_score` for read-only REST market/network scalar metadata.
...
- Targeted verification: `python -m pytest tests/tools/test_kaspa_tools.py -q` currently passes with 59 tests.
